Attribute VB_Name = "Module99_Misc"
Sub Clear_All_Highlighting_Globally()
'=============================================================================
' PURPOSE: Forcefully strips all background text highlighting across every
'          single structural layer, including stubborn unlinked headers/footers.
'=============================================================================
    Dim doc As Document
    Dim sec As Section
...
Sub Selection_Trim_Multiple_Spaces()
'=============================================================================
' Name: Selection_Trim_Multiple_Spaces
...
Sub Purge_Ghost_TOC_Levels()
    ...
    ' Loop through all paragraphs to catch hidden layout overrides
    For Each para In doc.Paragraphs
        ' Check if it's outside a table and has a heading outline level
        If Not para.Range.Information(wdWithInTable) Then
            If para.OutlineLevel >= wdOutlineLevel1 And para.OutlineLevel <= wdOutlineLevel3 Then
                ' If it's NOT styled with a native Heading style, reset its outline property
    ...
    ' Instantly update the TOC field code to show the clean layout
...
Sub Reset_All_List_Style_Links()
    ...
    ' Stop Word from visually rendering changes to maximize speed
    Application.ScreenUpdating = False
    
    On Error Resume Next ' Safety net for protected or read-only system structures
    
    ' Loop through every list template stored in the document canvas
    For Each lt In doc.ListTemplates
        ' Deep-scan all 9 available levels in the multilevel list hierarchy
        For i = 1 To 9
            Set lvl = lt.ListLevels(i)
            
            ' Strip the style anchor link completely
            lvl.LinkedStyle = ""
            
        Next i
    Next lt
    
    ' Forcefully update all Tables of Contents to reflect the clean layout immediately
    Dim toc As TableOfContents
    For Each toc In doc.TablesOfContents
        toc.Update
    Next toc
