...
' Purpose: Executes a 4-phase optimization pipeline:
'          1. Direct formats all body text to clear unmanaged style drifts.
'          2. Restores tight 1.0 spacing and 0/0 padding to all Tables.
'          3. Purges empty "ghost" heading lines from the Navigation Pane.
'          4. Loops headings, dropping Phase 1 font overrides via .Font.Reset.
'=============================================================================
    Dim doc As Document
    Dim tbl As Table
    ...
    ' Speed optimization: Turn off screen updates and animations
    Application.ScreenUpdating = False
    ...
    '-------------------------------------------------------------------------
    ' PHASE 3: RESTORE MASTER HEADING STYLES
    '-------------------------------------------------------------------------
    ' Rebuild each heading paragraph as a full paragraph-level style update so
    ' Word reapplies both character and paragraph formatting consistently.
    For lvl = 1 To 4
        headingStyleName = "Heading " & lvl

        Dim paraStyleName As String
        For Each para In doc.Paragraphs
            paraStyleName = ""
            On Error Resume Next
            paraStyleName = CStr(para.Style)
            On Error GoTo 0

            If StrComp(paraStyleName, headingStyleName, vbTextCompare) = 0 Then
                para.Range.Font.Reset
                para.Range.Style = headingStyleName
            End If
        Next para
    Next lvl

CleanUp:
    ' Restore system rendering and alert the user
    Application.ScreenUpdating = True
    MsgBox "Document successfully standardized, tables preserved, and ghost headings purged!", vbInformation, "Process Complete"
    Exit Sub
